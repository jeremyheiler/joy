...
#define THREEPARAMS(NAME)                                                      \
        if (stk == NULL || stk->next == NULL || stk->next->next == NULL)       \
        {                                                                      \
...
#define NUMERICTYPE(NAME)                                                      \
        if (stk->op != INTEGER_ && stk->op != CHAR_ && stk->op != BOOLEAN_)    \
        {                                                                      \
...
#define FLOATABLE (stk->op == INTEGER_ || stk->op == FLOAT_)
...
#define FLOAT2(NAME)                                                           \
        if (!(FLOATABLE2                                                       \
              || (stk->op == INTEGER_ && stk->next->op == INTEGER_)))          \
        {                                                                      \
...
#define NULLARY(CONSTRUCTOR, VALUE) stk = CONSTRUCTOR(VALUE, stk)
#define UNARY(CONSTRUCTOR, VALUE) stk = CONSTRUCTOR(VALUE, stk->next)
#define BINARY(CONSTRUCTOR, VALUE) stk = CONSTRUCTOR(VALUE, stk->next->next)
#define GNULLARY(TYPE, VALUE) stk = newnode(TYPE, (VALUE), stk)
#define GUNARY(TYPE, VALUE) stk = newnode(TYPE, (VALUE), stk->next)
#define GBINARY(TYPE, VALUE) stk = newnode(TYPE, (VALUE), stk->next->next)
...
#define MAXMIN(PROCEDURE, NAME, OPER)                                          \
        ...
        {                                                                      \
                ...
                if (stk->op == CHAR_)                                          \
                {                                                              \
                        BINARY(CHAR_NEWNODE, stk->u.num OPER stk->next->u.num  \
                                                 ? stk->next->u.num            \
                                                 : stk->u.num);                \
                }                                                              \
                else                                                           \
        ...
        {                                                                      \
                ...
                {                                                              \
                        ...
                        {                                                      \
                                ...
                                comp = strcmp(GETSTRING(stk->next),            \
                                              GETSTRING(stk)) OPR 0;           \
                                break;                                         \
                        }                                                      \
                }                                                              \
        ...
        {                                                                      \
                ...
                {                                                              \
                        ...
                        {                                                      \
                                BINARY(                                        \
                                    LIST_NEWNODE,                              \
                                    newnode(ELEM->op, ELEM->u, AGGR->u.lis));  \
                                break;                                         \
                        }                                                      \
                        case SET_:                                             \
                        {                                                      \
        ...
        switch (SAVED2->op)
        {
                case LIST_:
                {
                        ...
                        while (DMP1 != NULL)
                        {
                                ...
                                exeterm(SAVED1->u.lis);
                                D(printf("map: "); writefactor(stk, stdout);
                                  printf("\n"));
                                if (DMP2 == NULL) /* first */
                                {
        ...
        switch (SAVED2->op)
        {
                ...
                case LIST_:
                {
                        ...
                        while (DMP1 != NULL)
                        {
                                stk = newnode(DMP1->op, DMP1->u, SAVED3);
                                exeterm(SAVED1->u.lis);
                                D(printf("filter: "); writefactor(stk, stdout);
                                  printf("\n");)
                                if (stk->u.num) /* test */
                                {
                                        if (DMP2 == NULL) /* first */
                                        {
                                                DMP2 = newnode(DMP1->op,
                                                               DMP1->u, NULL);
                                                DMP3 = DMP2;
                                        }
                                        else /* further */
        ...
        switch (SAVED2->op)
        {
                ...
                case LIST_:
                {
                        ...
                        while (DMP1 != NULL)
                        {
                                stk = newnode(DMP1->op, DMP1->u, SAVED3);
                                exeterm(SAVED1->u.lis);
                                D(printf("split: "); writefactor(stk, stdout);
                                  printf("\n");)
                                if (stk->u.num) /* pass */
                                {
                                        if (DMP2 == NULL) /* first */
...
#define SOMEALL(PROCEDURE, NAME, INITIAL)                                      \
        ...
        {                                                                      \
                ...
                {                                                              \
                        ...
                        {                                                      \
                                long j;                                        \
                                for (j = 0; j < SETSIZE && result == INITIAL;  \
                                     j++)                                      \
                                {                                              \
                                        if (SAVED2->u.set & (1 << j))          \
                                        {                                      \
                                                stk = INTEGER_NEWNODE(j,       \
                                                                      SAVED3); \
                                                exeterm(SAVED1->u.lis);        \
...
static struct
{
    ...
    /* THESE MUST BE DEFINED IN THE ORDER OF THEIR VALUES */
    {
        ...
        {" truth value type", dummy_, "->  B", "The logical type, or the type "
                                               "of truth values.\nIt has just "
        ...
        {"maxint", maxint_, "->  maxint", "Pushes largest integer (platform "
                                          "dependent). Typically it is 32 "
                                          "bits."},

        {"setsize", setsize_, "->  setsize",
         "Pushes the maximum number of elements in a set (platform "
         "dependent).\nTypically it is 32, and set members are in the range "
         "0..31."},
        ...
        {"clock", clock_, "->  I", "Pushes the integer value of current CPU "
                                   "usage in hundreds of a second."},
        ...
        {"id", id_, "->", "Identity function, does nothing.\nAny program of "
                          "the form  P id Q  is equivalent to just  P Q."},
        ...
        {"or", or_, "X Y  ->  Z", "Z is the union of sets X and Y, logical "
                                  "disjunction for truth values."},

        {"xor", xor_, "X Y  ->  Z", "Z is the symmetric difference of sets X "
                                    "and Y,\nlogical exclusive disjunction for "
                                    "truth values."},

        {"and", and_, "X Y  ->  Z", "Z is the intersection of sets X and Y, "
                                    "logical conjunction for truth values."},

        {"not", not_, "X  ->  Y",
         "Y is the complement of set X, logical negation for truth values."},

        {"+", plus_, "M I  ->  N", "Numeric N is the result of adding integer "
                                   "I to numeric M.\nAlso supports float."},

        {"-", minus_, "M I  ->  N", "Numeric N is the result of subtracting "
                                    "integer I from numeric M.\nAlso supports "
                                    "float."},

        {"*", mul_, "I J  ->  K",
         "Integer K is the product of integers I and J.  Also supports float."},

        {"/", divide_, "I J  ->  K", "Integer K is the (rounded) ratio of "
                                     "integers I and J.  Also supports float."},

        {"rem", rem_, "I J  ->  K", "Integer K is the remainder of dividing I "
                                    "by J.  Also supports float."},

        {"div", div_, "I J  ->  K L",
         "Integers K and L are the quotient and remainder of dividing I by J."},

        {"sign", sign_, "N1  ->  N2", "Integer N2 is the sign (-1 or 0 or +1) "
                                      "of integer N1,\nor float N2 is the sign "
                                      "(-1.0 or 0.0 or 1.0) of float N1."},

        {"neg", neg_, "I  ->  J",
         "Integer J is the negative of integer I.  Also supports float."},

        {"ord", ord_, "C  ->  I", "Integer I is the Ascii value of character C "
                                  "(or logical or integer)."},

        {"chr", chr_, "I  ->  C", "C is the character whose Ascii value is "
                                  "integer I (or logical or character)."},

        {"abs", abs_, "N1  ->  N2", "Integer N2 is the absolute value "
                                    "(0,1,2..) of integer N1,\nor float N2 is "
                                    "the absolute value (0.0 ..) of float N1"},
        ...
        {"frexp", frexp_, "F  ->  G I", "G is the mantissa and I is the "
                                        "exponent of F.\nUnless F = 0, 0.5 <= "
                                        "abs(G) < 1.0."},
        ...
        {"modf", modf_, "F  ->  G H", "G is the fractional part and H is the "
                                      "integer part\n(but expressed as a "
                                      "float) of F."},
        ...
        {"localtime", localtime_, "I  ->  T",
         ...
         "month day hour minute second isdst yearday weekday].\nMonth is 1 = "
         "January ... 12 = December;\nisdst is a Boolean flagging daylight "
         "savings/summer time;\nweekday is 0 = Monday ... 7 = Sunday."},

        {"gmtime", gmtime_, "I  ->  T",
         "Converts a time I into a list T representing universal time:\n[year "
         "month day hour minute second isdst yearday weekday].\nMonth is 1 = "
         "January ... 12 = December;\nisdst is false; weekday is 0 = Monday "
         "... 7 = Sunday."},

        {"mktime", mktime_, "T  ->  I", "Converts a list T representing local "
                                        "time into a time I.\nT is in the "
                                        "format generated by localtime."},

        {"strftime", strftime_, "T S1  ->  S2",
         "Formats a list T in the format of localtime or gmtime\nusing string "
         "S1 and pushes the result S2."},

        {"strtol", strtol_, "S I  ->  J",
         "String S is converted to the integer J using base I.\nIf I = 0, "
         "assumes base 10,\nbut leading \"0\" means base 8 and leading \"0x\" "
         "means base 16."},
        ...
        {"formatf", formatf_, "F C I J  ->  S",
         ...
         "'f = fractional,\n'g or G = general with lower or upper case "
        ...
        {"fopen", fopen_, "P M  ->  S",
         "The file system object with pathname P is opened with mode M (r, w, "
         "a, etc.)\nand stream object S is pushed; if the open fails, "
         "file:NULL is pushed."},

        {"fread", fread_, "S I  ->  S L", "I bytes are read from the current "
                                          "position of stream S\nand returned "
                                          "as a list of I integers."},

        {"fwrite", fwrite_, "S L  ->  S", "A list of integers are written as "
                                          "bytes to the current position of "
                                          "stream S."},
        ...
        {"fputchars", fputchars_, "S \"abc..\"  ->  S",
         "The string abc.. (no quotes) is written to the current position of "
         "stream S."},
        ...
        {"fseek", fseek_, "S P W  ->  S",
         "Stream S is repositioned to position P relative to whence-point "
         "W,\nwhere W = 0, 1, 2 for beginning, current position, end "
         "respectively."},
        ...
        {"take", take_, "A N  ->  B", "Aggregate B is the result of retaining "
                                      "just the first N elements of A."},
        ...
        {"enconcat", enconcat_, "X S T  ->  U",
         "Sequence U is the concatenation of sequences S and T\nwith X "
         "inserted between S and T (== swapd cons concat)"},

        {"name", name_, "sym  ->  \"sym\"",
         "For operators and combinators, the string \"sym\" is the name of "
        ...
        {">=", geql_, "X Y  ->  B", "Either both X and Y are numeric or both "
                                    "are strings or symbols.\nTests whether X "
                                    "greater than or equal to Y.  Also "
                                    "supports float."},

        {">", greater_, "X Y  ->  B",
         "Either both X and Y are numeric or both are strings or "
         "symbols.\nTests whether X greater than Y.  Also supports float."},

        {"<=", leql_, "X Y  ->  B", "Either both X and Y are numeric or both "
                                    "are strings or symbols.\nTests whether X "
                                    "less than or equal to Y.  Also supports "
                                    "float."},

        {"<", less_, "X Y  ->  B", "Either both X and Y are numeric or both "
                                   "are strings or symbols.\nTests whether X "
                                   "less than Y.  Also supports float."},

        {"!=", neql_, "X Y  ->  B", "Either both X and Y are numeric or both "
                                    "are strings or symbols.\nTests whether X "
                                    "not equal to Y.  Also supports float."},

        {"=", eql_, "X Y  ->  B", "Either both X and Y are numeric or both are "
                                  "strings or symbols.\nTests whether X equal "
                                  "to Y.  Also supports float."},
        ...
        {"construct", construct_, "[P] [[P1] [P2] ..]  ->  R1 R2 ..",
         "Saves state of stack and then executes [P].\nThen executes each [Pi] "
         "to give Ri pushed onto saved stack."},
        ...
        {"ternary", ternary_, "X Y Z [P]  ->  R",
         "Executes P, which leaves R on top of the stack.\nNo matter how many "
         "parameters this consumes,\nexactly three are removed from the "
         "stack."},
        ...
        {"binrec", binrec_, "[B] [T] [R1] [R2]  ->  ...",
         "Executes P. If that yields true, executes T.\nElse uses R1 to "
         "produce two intermediates, recurses on both,\nthen executes R2 to "
         "combines their results."},

        {"genrec", genrec_, "[B] [T] [R1] [R2]  ->  ...",
         "Executes B, if that yields true executes T.\nElse executes R1 and "
         "then [[B] [T] [R1] [R2] genrec] R2."},

        {"condnestrec", condnestrec_, "[ [C1] [C2] .. [D] ]  ->  ...",
         "A generalisation of condlinrec. Each [Ci] is of the form [[B] [R1] "
         "[R2] .. [Rn]] and [D] is of the form [[R1] [R2] .. [Rn]]. Tries each "
         "B, or if all fail, takes the default [D]. For the case taken, "
         "executes each [Ri] but recurses between any two consecutive [Ri]. (n "
         "> 3 would be exceptional.)"},

        {"condlinrec", condlinrec_, "[ [C1] [C2] .. [D] ]  ->  ...",
         "Each [Ci] is of the forms [[B] [T]] or [[B] [R1] [R2]].\nTries each "
         "B. If that yields true and there is just a [T], executes T and "
         "exit.\nIf there are [R1] and [R2], executes R1, recurses, executes "
         "R2.\nSubsequent case are ignored. If no B yields true, then [D] is "
         "used.\nIt is then of the forms [[T]] or [[R1] [R2]]. For the former, "
         "executes T.\nFor the latter executes R1, recurses, executes R2."},

        {"step", step_, "A  [P]  ->  ...", "Sequentially putting members of "
                                           "aggregate A onto stack,\nexecutes "
                                           "P for each member of A."},

        {"fold", fold_, "A V0 [P]  ->  V",
         "Starting with value V0, sequentially pushes members of aggregate "
         "A\nand combines with binary operator P to produce value V."},

        {"map", map_, "A [P]  ->  B", "Executes P on each member of aggregate "
                                      "A,\ncollects results in sametype "
                                      "aggregate B."},
        ...
        {"infra", infra_, "L1 [P]  ->  L2",
         ...
         "first element of L1 is used as the top of stack,\nand after "
         "execution of P the top of stack becomes the first element of L2."},

        {"primrec", primrec_, "X [I] [C]  ->  R",
         "Executes I to obtain an initial value R0.\nFor integer X uses "
         "increasing positive integers to X, combines by C for new R.\nFor "
         "aggregate X uses successive members and combines by C for new R."},

        {"filter", filter_, "A [B]  ->  A1",
         "Uses test B to filter aggregate A producing sametype aggregate A1."},

        {"split", split_, "A [B]  ->  A1 A2", "Uses test B to split aggregate "
                                              "A into sametype aggregates A1 "
                                              "and A2 ."},
        ...
        {"treerec", treerec_, "T [O] [C]  ->  ...",
         "T is a tree. If T is a leaf, executes O. Else executes [[O] [C] "
         "treerec] C."},

        {"treegenrec", treegenrec_, "T [O1] [O2] [C]  ->  ...",
         "T is a tree. If T is a leaf, executes O1.\nElse executes O2 and then "
         "[[O1] [O2] [C] treegenrec] C."},

        /* MISCELLANEOUS */

        {"help", help1_, "->", "Lists all defined symbols, including those "
                               "from library files.\nThen lists all primitives "
                               "of raw Joy\n(There is a variant: \"_help\" "
                               "which lists hidden symbols)."},

        {"_help", h_help1_, "->", "Lists all hidden symbols in library and "
                                  "then all hidden inbuilt symbols."},
        ...
        {"__html_manual", html_manual_, "->", "Writes this manual of all Joy "
                                              "primitives to output file in "
                                              "HTML style."},

        {"__latex_manual", latex_manual_, "->",
         "Writes this manual of all Joy primitives to output file in Latex "
         "style but without the head and tail."},

        {"__manual_list", manual_list_aux_, "->  L",
         "Pushes a list L of lists (one per operator) of three documentation "
         "strings"},

        {"__settracegc", settracegc_, "I  ->",
         "Sets value of flag for tracing garbage collection to I (= 0..5)."},

        {"setautoput", setautoput_, "I  ->", "Sets value of flag for automatic "
                                             "put to I (if I = 0, none;\nif I "
                                             "= 1, put; if I = 2, stack."},
        ...
        {"setecho", setecho_, "I ->", "Sets value of echo flag for listing.\nI "
                                      "= 0: no echo, 1: echo, 2: with tab, 3: "
                                      "and linenumber."},
        ...
        {"system", system_, "\"command\"  ->",
         ...
         "execution of another program.\nWhen that has finished, the process "
         "returns to Joy."},

        {"getenv", getenv_, "\"variable\"  ->  \"value\"",
         "Retrieves the value of the environment variable \"variable\"."},

        {"argv", argv_, "-> A", "Creates an aggregate A containing the "
                                "interpreter's command line arguments."},

        {"argc", argc_, "-> I", "Pushes the number of command line arguments. "
        ...
        {"include", include_, "\"filnam.ext\"  ->",
         ...
         "end-of-file returns to previous input file."},
